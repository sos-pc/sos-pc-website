...
function getWidget() {
  return document.getElementById("sospc-widget")!;
}
...
function initDrag() {
  [
    document.getElementById("sospc-fab-content"),
    document.getElementById("sospc-header"),
  ].forEach((el) => {
    if (!el) return;
    ...
    el.addEventListener("pointermove", (e: Event) => {
      ...
      if (
        Math.abs(pe.clientX - dragStartX) > 6 ||
        Math.abs(pe.clientY - dragStartY) > 6
      )
        hasMoved = true;
      if (hasMoved)
        positionWidget(
          pe.clientX - dragOffsetX,
          pe.clientY - dragOffsetY,
          false,
        );
    });
...
(window as any).sospcToggle = function () {
  ...
  if (isOpen) {
    ...
    if (diagData)
      setTimeout(() => {
        const inp = document.getElementById("sospc-input") as HTMLInputElement;
        inp?.focus();
      }, 300);
  } else {
  ...
  (document.getElementById("sospc-fab-badge") as HTMLElement).style.display =
    "none";
};
...
function clearState() {
  try {
    localStorage.removeItem(STORAGE_DIAG);
  } catch {}
}
...
(window as any).sospcReset = function () {
  if (confirm("Lancer un nouveau scan ?")) {
    ...
    (document.getElementById("sospc-view-diag") as HTMLElement).style.display =
      "none";
    (
      document.getElementById("sospc-view-waiting") as HTMLElement
    ).style.display = "block";
    (
      document.getElementById("sospc-waiting-indicator") as HTMLElement
    ).style.display = "none";
...
function startPolling() {
  ...
  pollInterval = setInterval(async () => {
    try {
      const res = await (
        await fetch(
          `https://sos-pc-diagnostic.netlify.app/api/poll?s=${sessionId}`,
        )
      ).json();
...
function showDiagView() {
  (document.getElementById("sospc-view-waiting") as HTMLElement).style.display =
    "none";
...
function buildFullReport() {
  ...
  if (sec.defender_enabled === false || sec.realtime_protection === false) {
    ...
    if (sec.defender_enabled === false)
      lines.push("  >> Antivirus Windows Defender INACTIF");
    if (sec.realtime_protection === false)
      lines.push("  >> Protection en temps réel INACTIVE");
    if (sec.antivirus_signature_date)
      lines.push("  >> Dernières signatures : " + sec.antivirus_signature_date);
    lines.push("");
  }
  ...
  const ramPct =
    os.ram_total_gb && os.ram_free_gb
      ? Math.round((1 - os.ram_free_gb / os.ram_total_gb) * 100)
      : "?";
  lines.push(
    "RAM libre    : " +
      (os.ram_free_gb || "?") +
  ...
  if (os.pagefile_total_gb)
    lines.push(
      "Pagefile     : " +
        os.pagefile_used_gb +
        " / " +
        os.pagefile_total_gb +
        " Go",
    );
  lines.push("");
  ...
  if (gpus.length > 0) {
    ...
    gpus.forEach((g: any) => {
      lines.push(
        "  " +
          (g.name || "?") +
          (g.ram_mb ? " -- " + g.ram_mb + " Mo VRAM" : "") +
          (g.driver ? " -- driver " + g.driver : "") +
          (g.driver_date ? " (" + g.driver_date + ")" : ""),
      );
      if (g.resolution)
        lines.push(
          "  Résolution : " +
            g.resolution +
            (g.refresh_hz ? " @ " + g.refresh_hz + "Hz" : ""),
        );
    });
    lines.push("");
  }
    ...
    physDisks.forEach((dk: any) => {
      ...
      lines.push(
        "  Santé : " +
          (dk.health_status || "?") +
          "  |  Heures : " +
          (dk.hours_used != null ? dk.hours_used + "h" : "?") +
          "  |  Temp : " +
          (dk.temperature_c != null ? dk.temperature_c + "°C" : "?"),
      );
      if (dk.reallocated_sectors)
        lines.push(
          "  ATTENTION : " + dk.reallocated_sectors + " secteurs défaillants",
        );
      if (dk.read_errors)
        lines.push("  ATTENTION : " + dk.read_errors + " erreurs de lecture");
    });
    lines.push("");
  }
  ...
  if (net.adapters && net.adapters.filter(Boolean).length > 0) {
    ...
    lines.push(
      "Internet     : " +
        (net.internet_ok ? "OK" : "ÉCHEC") +
        (net.dns_latency_ms ? "  (DNS: " + net.dns_latency_ms + "ms)" : ""),
    );
    net.adapters.filter(Boolean).forEach((a: any) => {
      lines.push(
        "  " +
          ...
          a.name +
          (a.ip ? " -- " + a.ip : "") +
          (a.speed_mbps ? " -- " + a.speed_mbps + " Mbps" : ""),
      );
    });
    lines.push("");
  }
  ...
  if (Object.keys(security).length > 0) {
    ...
    lines.push(
      "Antivirus    : " +
        (security.defender_enabled ? "Actif" : "INACTIF") +
        "  |  Temps réel : " +
        (security.realtime_protection ? "Actif" : "INACTIF"),
    );
    if (security.antivirus_signature_date)
      lines.push("Signatures   : " + security.antivirus_signature_date);
    lines.push(
      "Pare-feu     : Domaine " +
        (security.firewall_domain ? "OK" : "OFF") +
        " / Privé " +
        (security.firewall_private ? "OK" : "OFF") +
        " / Public " +
        (security.firewall_public ? "OK" : "OFF"),
    );
    lines.push(
      "UAC          : " + (security.uac_enabled ? "Actif" : "Inactif"),
    );
    lines.push("");
  }
  ...
  if (temps.length > 0) {
    ...
    temps.forEach((t: any) => {
      lines.push("  " + t.instance + " : " + t.temp_celsius + "°C");
    });
    lines.push("");
  }
  ...
  if (Object.keys(perf).length > 0) {
    ...
    if (perf.disk_pct_busy != null)
      lines.push("Disque I/O   : " + perf.disk_pct_busy + "%");
    if (perf.ram_used_pct != null)
      lines.push("RAM          : " + perf.ram_used_pct + "%");
    if (perf.pagefile_used_pct != null)
      lines.push("Pagefile     : " + perf.pagefile_used_pct + "%");
    lines.push("");
  }
  ...
  if (stability.bsod_count > 0) {
    ...
    lines.push(dash);
    (stability.bsod_last_7days || []).forEach((b: any) => {
      lines.push("  [" + b.time + "] ID:" + b.id + " -- " + b.message);
    });
    lines.push("");
  }
      ...
      const lvl =
        i.level === "critical"
          ...
          : i.level === "warning"
            ...
            : "[OK]       ";
      lines.push(
        lvl + " " + (i.category ? "[" + i.category + "] " : "") + i.title,
      );
  ...
  if (quickWins.length > 0) {
    ...
    quickWins.forEach((q: string, idx: number) => {
      lines.push("  " + (idx + 1) + ". " + q);
    });
    lines.push("");
  }
  ...
  if (userSymptoms.length > 0) {
    ...
    lines.push(dash);
    userSymptoms.forEach((s: string, idx: number) => {
      lines.push("  " + (idx + 1) + '. "' + s + '"');
    });
    lines.push("");
  }
  ...
  if (hotfixes.length > 0) {
    ...
    hotfixes.forEach((u: any) => {
      lines.push(
        "  " + u.id + " -- " + u.description + " (" + u.installed_on + ")",
      );
    });
    lines.push("");
  }
  ...
  lines.push(sep);
  lines.push(
    "  Rapport Talos Int. -- talos-int.com -- " +
...
(window as any).sospcAttachToForm = function () {
  ...
  window.dispatchEvent(
    new CustomEvent("sospc:attach-diag", { detail: report }),
  );
  ...
  if (btn) {
    ...
    btn.innerHTML =
      '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> Diagnostic joint !';
    setTimeout(() => {
      btn.classList.remove("attached");
      btn.innerHTML =
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg> Joindre au formulaire de contact';
    }, 2500);
  }
};
...
async function apiFetch(url: string, body: any): Promise<any> {
  ...
  if (!r.ok) {
    const errMsg = data.error || data.message || `Erreur serveur (${r.status})`;
...
function analyzeData() {
  ...
  apiFetch("https://sos-pc-diagnostic.netlify.app/api/analyze", {
    data: diagData,
    problem: "",
  })
    .then((data) => {
      ...
      const criticals = (diagReport?.issues || []).filter(
        (i: any) => i.level === "critical",
      );
      if (criticals.length) {
        const msg =
          "⚠ " +
          ...
          criticals.map((i: any) => i.title).join(", ") +
          ".";
        addMessage("bot", msg);
      ...
      if ((diagReport?.issues || []).some((i: any) => i.level !== "ok")) {
        const msg =
          "Nos techniciens peuvent résoudre ces problèmes. Voulez-vous être contacté ?";
        addMessage("bot", msg);
        chatHistory.push({ role: "assistant", content: msg });
      }
    ...
    .catch((err: any) => {
      ...
      addMessage(
        "bot",
        "Erreur d'analyse : " +
          (err.message || "Contactez-nous au 07 69 56 14 91."),
      );
    });
}
...
(window as any).sospcSend = function () {
  ...
  apiFetch("https://sos-pc-diagnostic.netlify.app/api/chat", {
    ...
    .then((data) => {
      ...
      const msg =
        ...
        data.reply ||
        data.response ||
        "Erreur : réponse vide du serveur.";
      addMessage("bot", msg);
      chatHistory.push({ role: "assistant", content: msg });
      saveState();
      window.dispatchEvent(
        new CustomEvent("sospc:attach-diag-update", {
          detail: buildFullReport(),
        }),
      );
    })
    .catch((err: any) => {
      ...
      addMessage("bot", "Erreur : " + (err.message || "Erreur de connexion."));
    })
    .finally(() => {
      (document.getElementById("sospc-send") as HTMLButtonElement).disabled =
        false;
      input.focus();
    });
};
